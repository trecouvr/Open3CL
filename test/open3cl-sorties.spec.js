...
import { getAdemeFileJson, getResultFile } from './test-helpers.js';
import { buildExpectedObject_closeToPercent } from './test-utils.js';
...
describe('Test Open3CL engine compliance on corpus', () => {
  ...
  describe('check "apport_et_besoin" value', () => {
    test.each(corpus)('dpe %s', (ademeId) => {
      const expectedDpe = getAdemeFileJson(ademeId).logement.sortie.apport_et_besoin;
      const calculatedDpe = getResultFile(ademeId).logement.sortie.apport_et_besoin;
      const expected = buildExpectedObject_closeToPercent(expectedDpe, [
      ...
      expect(calculatedDpe).toMatchObject(expected);
    });
  });
